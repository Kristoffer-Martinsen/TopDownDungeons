...
var ASTAR_DIMENSIONS: Rect2i

func _ready():
	SignalBus.dungeon_tileset_generated.connect(_create_astar2d_grid)

func _get_tile_data() -> void:
	for x in range(ASTAR_DIMENSIONS.position.x, ASTAR_DIMENSIONS.position.x + ASTAR_DIMENSIONS.size.x):
		for y in range(ASTAR_DIMENSIONS.position.y, ASTAR_DIMENSIONS.position.y + ASTAR_DIMENSIONS.size.y):
